                        ...
                        <td className="px-5 py-4">
                          <div className="flex gap-3">
                            ...
                            <div className="min-w-0">
                              ...
                              <p className="mt-1 truncate font-mono text-xs text-slate-500">{line.product.barcode}{quote?.gstApplied ? ` · GST ${line.product.gstRate}%` : ""}</p>
        ...
        <aside className="flex min-w-0 flex-col gap-4">
          ...
          <section className="md-pos-checkout flex flex-1 flex-col rounded-2xl border p-5 shadow-sm">
            ...
            <div className="mt-auto border-t border-slate-200 pt-5">
              ...
              <p className="mt-2 text-right text-xs text-slate-400">{quoting ? "Recalculating…" : !quote?.gstApplied ? "Simple bill · shop GST not configured" : quote.pricesIncludeGst ? "Prices include GST" : "GST added to prices"}</p>
...
export function ThermalReceipt({
  ...
  const width = invoice.store.receiptWidth === "MM_58" ? 58 : 80;
  const template = invoice.store.thermalReceiptTemplate ?? "CLASSIC";
  const compact = width === 58 || template === "COMPACT";
  const bordered = template === "BORDERED";
  const customer = invoice.payments.find((payment) => payment.customerId);
  const gstApplied = invoice.totals.gstApplied;
  return (
    <article className={`thermal-receipt bg-white font-mono text-black ${bordered ? "border-2 border-black" : ""}`} style={{ width: `${width}mm`, padding: compact ? "2.5mm" : bordered ? "3.5mm" : "4mm", fontSize: compact ? "7.5pt" : "9pt", lineHeight: compact ? 1.22 : 1.35 }}>
      <header className={`text-center ${bordered ? "border-b-2 border-black pb-2" : ""}`}>{logoUrl && <img src={logoUrl} alt="" className="mx-auto mb-1 object-contain grayscale" style={{ maxWidth: compact ? "28mm" : "42mm", maxHeight: compact ? "9mm" : "15mm", filter: "grayscale(1) contrast(1.2)" }} />}<h1 className={`font-sans font-black ${compact ? "text-sm" : "text-base"}`}>{invoice.store.shopName}</h1><p>{invoice.store.address}</p><p>Phone: {invoice.store.phone}</p>{invoice.store.gstin && <p>GSTIN: {invoice.store.gstin}</p>}{duplicate && <p className="mt-1 border-y border-black py-0.5 font-sans text-xs font-black tracking-widest">DUPLICATE COPY</p>}</header>
